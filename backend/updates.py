    ...
    *,
    ...
    return "\n".join(
        [
            ...
            f"$installerLog = {_ps_single_quote(str(log.with_name('installer.log')))}",
            ...
            "    ForEach-Object { try { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue } catch {} }",
            "}",
            "function Test-RunningExecutable([string]$Path) {",
            ...
            "    $webviewLock = Join-Path $webviewDir 'EBWebView\\lockfile'",
            "    $webviewDeadline = (Get-Date).AddSeconds(30)",
            "    while (Test-LockedFile $webviewLock -and ((Get-Date) -lt $webviewDeadline)) {",
            "      Start-Sleep -Seconds 1",
            "    }",
            "    if (Test-LockedFile $webviewLock) {",
            "      Stop-GradeCalculatorWebView $webviewDir",
            "      Start-Sleep -Seconds 2",
            "    }",
            "    if (Test-LockedFile $webviewLock) {",
            "      throw 'The embedded browser is still shutting down. Please try the update again.'",
            "    }",
            "  }",
            "  $process = Start-Process -FilePath $installer -ArgumentList @('/SP-', '/VERYSILENT', '/SUPPRESSMSGBOXES', '/NORESTART', '/NORESTARTAPPLICATIONS', '/CLOSEAPPLICATIONS', '/FORCECLOSEAPPLICATIONS', ('/LOG=' + $installerLog)) -PassThru",
            "  $process.WaitForExit()",
            "  if ($process.ExitCode -eq 0) {",
            "    Write-Status 'applied' 'Update installed'",
            "    if (-not (Test-RunningExecutable $executable)) {",
            ...
            "      $details = (Get-Content -LiteralPath $installerLog -Tail 24 -ErrorAction SilentlyContinue | Out-String).Trim()",
            "      if ($details) { $message = $message + [Environment]::NewLine + $details }",
            "    }",
